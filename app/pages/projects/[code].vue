...
const activity = [{ text: 'Иван Соколов обновил задачу MOB-142', time: '12 минут назад', icon: 'i-lucide-refresh-cw' }, { text: 'В staging опубликована версия 2.3.2', time: '2 часа назад', icon: 'i-lucide-rocket' }, { text: 'Обновлена ссылка на Figma', time: 'Вчера', icon: 'i-lucide-link-2' }]
...
const tabs = [{ value: 'about', label: 'Проект', icon: 'i-lucide-layout-dashboard' }, { value: 'modules', label: 'Модули', icon: 'i-lucide-box' }, { value: 'tasks', label: 'Задачи', icon: 'i-lucide-list-checks' }, { value: 'releases', label: 'Релизы', icon: 'i-lucide-rocket' }, { value: 'environments', label: 'Среды', icon: 'i-lucide-server' }, { value: 'artifacts', label: 'Артефакты', icon: 'i-lucide-link-2' }, { value: 'risks', label: 'Риски', icon: 'i-lucide-triangle-alert' }, { value: 'activity', label: 'Активность', icon: 'i-lucide-history' }]
</script>
  ...
  <UDashboardPanel id="project-detail">
    ...
    <template #body>
      <div class="flex flex-col gap-6">
        ...
        <section v-else-if="activeTab === 'risks'" role="tabpanel"><UCard variant="subtle"><div class="flex items-center justify-between"><div><h2 class="font-semibold text-highlighted">Риски и блокеры</h2><p class="mt-1 text-sm text-muted">Контроль проблем, которые могут повлиять на проект</p></div><UButton icon="i-lucide-plus" label="Добавить риск" variant="soft" /></div><div class="mt-5 divide-y divide-default"><div v-for="risk in risks" :key="risk.title" class="flex items-center justify-between gap-4 py-4 first:pt-0 last:pb-0"><div><p class="text-sm font-medium text-highlighted">{{ risk.title }}</p><p class="mt-1 text-xs text-muted">{{ risk.owner }} · Влияние: {{ risk.impact }}</p></div><UBadge color="warning" variant="subtle">{{ risk.status }}</UBadge></div></div></UCard></section>

        <section v-else-if="activeTab === 'activity'" role="tabpanel"><UCard variant="subtle"><div><h2 class="font-semibold text-highlighted">Активность проекта</h2><p class="mt-1 text-sm text-muted">Изменения в проекте и события синхронизации</p></div><div class="mt-5 divide-y divide-default"><div v-for="item in activity" :key="item.text" class="flex gap-3 py-4 first:pt-0 last:pb-0"><span class="flex size-8 shrink-0 items-center justify-center rounded-full bg-elevated"><UIcon :name="item.icon" class="size-4 text-primary" /></span><div><p class="text-sm text-highlighted">{{ item.text }}</p><p class="mt-1 text-xs text-muted">{{ item.time }}</p></div></div></div></UCard></section>
